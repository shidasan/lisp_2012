...
push:
    sp_value->type = NUM;
    (sp_value++)->ivalue = pc->op[0].ivalue;
    goto *((++pc)->instruction_ptr);

plus2:
    (sp_value - 1)->ivalue += pc->op[0].ivalue;
    goto *((++pc)->instruction_ptr);
...
arg:
    sp_value->type = NUM;
    (sp_value++)->ivalue = (sp_arg-1)->ivalue;
    goto *((++pc)->instruction_ptr);

narg:
    sp_value->type = NUM;
    (sp_value++)->ivalue = (sp_arg - pc->op[0].ivalue)->ivalue;
    goto *((++pc)->instruction_ptr);
