...
(defun render (byteImage width height numberOfSubSamples)
 (let* ((fimg (make-array (list (* width height 3))))
   ...
   (dotimes (y height)
    (dotimes (x width)
     (dotimes (v numberOfSubSamples)
      (dotimes (u numberOfSubSamples)
       (system::svstore isect 1 (vector 
                        (/ (+ x (- (/ u numberOfSubSamples) (/ width 2.0))) (/ width 2.0))
                        (- (/ (+ y (- (/ v numberOfSubSamples) (/ height 2.0))) (/ height 2.0)))
                        (- 1.0)))
       ...
       (if (svref isect 5)
        ...
        (setq p (* 3 (+ (* y width) x)))
        (when (null (svref fimg p))
         (system::svstore fimg p 0)
         (system::svstore fimg (+ p 1) 0)
         (system::svstore fimg (+ p 2) 0))
     ...
     (when (null (svref fimg p1))
      (system::svstore fimg p1 0)
      (system::svstore fimg (+ p1 1) 0)
      (system::svstore fimg (+ p1 2) 0))
